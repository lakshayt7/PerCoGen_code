...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        control_image = self.processor(example["instance_images"].permute(1, 2, 0), hand_and_face=True)
        
        control_image = self.conditioning_transforms(control_image)
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    condition_values = [example["conditioning_pixel_values"] for example in examples]
...
def main(args):
    ...
    if args.train_text_encoder:
        params_to_optimize = (
            itertools.chain(unet.parameters(), controlnet.parameters(), text_encoder.parameters())
        )
    else:
        params_to_optimize = (
    ...
    if args.train_text_encoder:
        unet, controlnet, text_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, controlnet, text_encoder, optimizer, train_dataloader, lr_scheduler
        )
    else:
            ...
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(unet.parameters(), controlnet.parameters(), text_encoder.parameters())
                        if args.train_text_encoder
                        else itertools.chain(unet.parameters(), controlnet.parameters())
                    )
